...
# finer and finer meshes, we will solve our PDE using our finite element code and check that our numerical
...
# ### Exercise 5
#
# Derive the weak formulation of the partial differential equation. You will need the following integration by parts result.
#
# For functions $u, v \in H^1(\Omega)$ the following integration by parts result holds
#
# $$
...
#
# A uniform mesh $\mathcal{T}_h$ of $\Omega$ is a partitioning of the mesh constructed from $N$ disjoint cells $K_i$
#
# $$
...
#
# The standard hat functions $\{ \varphi_j \}_{j=0}^N$ form a basis for $V_h$. Each basis function $\varphi_j$ has the property
#
# $$
...
# ### Exercise 6
#
# The interpolation error estimate states that the error between the exact solution $u_{\mathrm{exact}} := u \in H^2$ and its interpolant $I_h u \in V_h$ is given by
#
# $$
